...
public:
	...
	// --- Hỗ trợ đọc nút nhấn (polling mỗi 1ms) ---
	// Quy ước mặc định: nút Active-Low (kéo xuống = nhấn), phát hiện cạnh xuống là sự kiện "nhấn".
	// Sử dụng theo chu kỳ 1ms: gọi Button_Tick_1ms() mỗi 1ms, sau đó đọc các cờ sự kiện.
	// - Button_Was_Pressed(): trả về true đúng 1 lần tại thời điểm mới nhấn (cạnh xuống)
	// - Button_Was_Held(): trả về true đúng 1 lần khi thời gian giữ >= hold_ms (mặc định 2000ms)
	// - Button_Was_Released(): trả về true đúng 1 lần khi thả nút
	// - Button_Is_Down(): trạng thái tức thời (đã qua debounce)
	// Gợi ý: cấu hình chân input với Pull-Up nếu dùng active-low.
	...
	// API đơn giản: gọi theo chu kỳ bất kỳ, truyền vào dt_ms (khoảng cách giữa 2 lần đọc) và trả về sự kiện
	// Không debounce. Quy ước: ưu tiên trả về PRESS tại cạnh xuống, HOLD sẽ trả về đúng 1 lần khi tổng thời gian giữ >= hold_ms.
	enum class ButtonEvent { NONE = 0, PRESS, HOLD };
